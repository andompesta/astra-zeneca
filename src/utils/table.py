...
class Table:
    ...
    #         ] for r in db.query('SELECT * from {}'.format(cls.get_id(table_id)))
    #                ]
    ...
    #             return
    #     type_str = ', '.join(
    #         ['col{} {}'.format(i, t) for i, t in enumerate(self.types)])
    #     db.query('CREATE TABLE {name} ({types})'.format(name=self.name,
    #                                                     types=type_str))
    #     for row in self.rows:
    #         value_str = ', '.join(
    #             [':val{}'.format(j) for j, c in enumerate(row)])
    ...
    #             }
    #         db.query(
    #             'INSERT INTO {name} VALUES ({values})'.format(name=self.name,
    #                                                           values=value_str),
    #             **value_dict)
    ...
    #     where_str = ' AND '.join([
    #         'col{} {} :col{}'.format(i, Query.cond_ops[o], i)
    ...
    #             agg_str=agg_str, name=self.name, where_str=where_str)
    #         return [[
    #             getattr(r, 'col{}'.format(i)) for i in range(len(self.header))
    ...
    #     where_str = ' AND '.join([
    #         '{} {} {}'.format(self.header[i], Query.cond_ops[o], v)
    #         for i, o, v in query.conditions
    #     ])
    #     return 'SELECT {} FROM {} WHERE {}'.format(agg_str, self.name,
    #                                                where_str)
    ...
    #         if [r[sel_index] for r in new_results
    #            ] != [r[sel_index] for r in results]:
